...
public class CaesarCipheDecrypt {
    ...
    public static void testCaesarOneKeyDecrypt(){
       ...
       // String message = fr.asString();
        String decrypted = decryptOneKey("Rpc ndj xbpvxct axut LXIWDJI iwt xcitgcti PCS rdbejitgh xc ndjg edrzti tttttttttt?");
    ...
    private static HashMap parceInputString(String input){
    ...
    private static char findFrequencyChar(String input){
        HashMap hashmap = parceInputString(input);
        char maxChar = 0;
        int max = 0;
        for (int k = 0; k < 26; k++){
            char chAlphK = alphabet.toLowerCase().charAt(k);
            if (hashmap.containsKey(chAlphK)){
                int v1 = (int) hashmap.get(chAlphK);
                if (max < v1){
                    max = v1;
                    maxChar = chAlphK;
                }
    ...
    private static int getKey(String input){
        char maxChar = findFrequencyChar(input);
    ...
    public static String decryptOneKey(String input){
     ...
     public static String twoKeyDecrypt(String input){
        ...
        for (int i = 0; i < input.length(); i++){
            if (i%2 == 0){
                firstKeyHashmap = parceInputString(input);
            } if (i%2 != 0){
                secondKeyHashmap = parceInputString(input);
            }
        }
